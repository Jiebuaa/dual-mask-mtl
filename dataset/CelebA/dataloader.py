...
class CelebaGroupedDataset(VisionDataset):
    ...
    def __init__(self, data_dir, task_groups, split='train', image_size=64, transform=None):
        rep_file = os.path.join(data_dir, 'list_eval_partition.txt')
        self.img_dir = os.path.join(data_dir, 'img_align_celeba/img_align_celeba/')
        self.ann_file = os.path.join(data_dir, 'list_attr_celeba.txt')
